...
function HeaderFooter({ className, webData }: DefaultHeaderProps) {
  const t = useTranslations('header');
  const combinedClassName = className ? `${styles.headerFooter} ${className}` : styles.headerFooter;
...
// DefaultHeaderWrapper remains unchanged
export default async function DefaultHeaderWrapper({ className }: { className?: string }) {
  const webData = await getWebData();
  return <HeaderFooter webData={webData} className={className} />;
}