...
### 4. 頁面資料獲取與快取策略：
   ...
   * **內容區塊類型自動偵測**：
       ...
       - **動態內容偵測**：
           * 檢查 API 回應中的標準快取標頭
           * `Last-Modified`：最後更新時間戳，用於判斷內容新鮮度
           * `Expires`：頁面過期時間戳，用於設定快取有效期
           * **快取策略決策**：
               - 若 `Expires` 時間較長（>1小時）→ 歸類為靜態內容，使用長期快取
               - 若 `Expires` 時間較短（≤30分鐘）→ 歸類為動態內容，使用短期快取
               - 若 `Last-Modified` 頻繁更新（與當前時間差距<10分鐘）→ 歸類為即時內容，使用極短期快取
       - **預設策略**：
           * 當 API 未提供快取標頭時，根據內容是否包含會員相關變數判斷
           * 無法確定內容類型時，使用保守的短期快取（5 分鐘）
   ...
   * **會員群組複選的快取策略**：
     ...
     **模組權限矩陣設計**：
       - **權限配置**：每個模組定義所需群組（requiredGroups）和排除群組（excludedGroups）
       - **權限邏輯**：
           * 無限制模組：所有使用者可見
           * 單一群組限制：需要特定群組成員資格
           * 多群組選擇：需要其中任一群組成員資格
     ...
     **分層快取架構**：
        * 第一層：API 原始資料快取
         - **快取內容**：API 回傳的完整原始資料（未經權限過濾）
         - **快取鍵值**：`api-raw-${project}-${language}-${uri}`
         - **快取時間**：依據 API 的 Expires 標頭（通常較長，1-2小時）
         - **適用範圍**：所有使用者共用的原始資料
         - **更新時機**：API 資料變更時清除

        * 第二層：權限過濾快取
            - **快取內容**：根據群組權限過濾後的模組和內容
            - **快取鍵值**：`filtered-${language}-${uri}-${groupHash}`
            - **快取時間**：中期快取（30分鐘）
            - **適用範圍**：相同群組組合的使用者共享
            - **處理邏輯**：
              * 從第一層快取取得原始資料
